...
describe "/apps/index.html.erb" do
  ...
  before(:each) do
    activites_chain = stub('activities', :null_object => true)
    activites_chain.stub!('latest').and_return([activities(:app_1_dev)])
    activites_chain.stub!('size').and_return(1)
    ...
    app_98.should_receive(:activities).exactly(2).and_return(activites_chain)
    ...
    app_99.should_receive(:activities).exactly(2).and_return(activites_chain)
